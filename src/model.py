...
def get_diffusion(args):
    if 'ffhq' in args.model_name_or_path:
        num_res_blocks = 1
        num_channels = 128
        num_heads = 4
    ...
    unet.load_state_dict(torch.load(args.model_name_or_path, weights_only=True))
    unet.to(args.device)
    unet.eval()
...
class OpenAIUNet2DModel(ModelMixin, ConfigMixin):
    config_name = "openai_unet_2d"
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if isinstance(timesteps, int):
            timesteps = torch.ones(x.shape[0], dtype=x.dtype, device=x.device) * timesteps
        elif len(timesteps.shape) == 0:
            timesteps = torch.ones(x.shape[0], dtype=x.dtype, device=x.device) * timesteps.item()
        ...
        for module in self.output_blocks:
            ...
            h = module(h, emb)
        h = h.type(x.dtype)
        sample = self.out(h)[:, :self.out_channels // 2]
        return UNet2DOutput(sample=sample)


class SuperResModel(OpenAIUNet2DModel):
    """
    A OpenAIUNet2DModel that performs super-resolution.
